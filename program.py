...
class Program:
    def __init__(self):
        ...
        self.main_menu_buttons_filled = create_dictionary_for_images("textures/buttons/menu/filled/",
                                                                     ["close", "editor", "load", "start"])
        self.hero_idle_right_pictures = [tkinter.PhotoImage(file=f"textures/hero/hero_idle_small/{i}.png") for i in
                                         range(0, 10)]

        self.canvas.pack()
        MainMenu(self.canvas, self.main_menu_bg_pictures, self.hero_idle_right_pictures, self.main_menu_buttons_normal,
